...
use App\Models\ReservationsIncItem;
...
#[Title('Food & Beverage | Record Income ')]
class ReservationsRecordIncome extends Component
{
    ...
    public $income_title;
    public $income_date;
    public $section  = 'Reservations';
    public $category_id;
    public $item_id;
    public $amount;
    public $note;
    public $income_id; // for edit case (optional)
    public $modal_title = 'Record an income';
    ...
    public function save()
    {
        try {
            ...
            // Get the item name from the ReservationsIncItem table
            $item = ReservationsIncItem::where('id', $this->item_id)->value('item');
   ...
   public function destroy($id)
{
...

    {
        $this->incomes = HotelIncome::where('section', $this->section)
            ->orderBy('id', 'desc')
            ->get();
        $items = ReservationsIncItem::orderBy("id", "desc")->get();
        $categories = ReservationsIncCategory::orderBy("id", "desc")->get();

        return view('livewire.reservations.reservations-record-income', [
            'items' => $items,
