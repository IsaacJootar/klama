...
use App\Models\FnbIncItem;
use App\Models\FnbIncCategory;
...
class FnbRecordIncome extends Component
{
    ...
    public function save()
    {
        try {
            ...
            // Get the item name from the FnbIncItem table
            $item = FnbIncItem::where('id', $this->item_id)->value('item');
   ...
   public function destroy($id)
{
...

    {
        $this->incomes = HotelIncome::where('section', $this->section)
            ->orderBy('id', 'desc')
            ->get();
        $items = FnbIncItem::orderBy("id", "desc")->get();
        $categories = FnbIncCategory::orderBy("id", "desc")->get();

        return view('livewire.fnb.fnb-record-income', [
            'items' => $items,
