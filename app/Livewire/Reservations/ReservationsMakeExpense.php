...
class ReservationsMakeExpense extends Component
{
    ...
    public function save()
    {
        try {
            ...
            // Get the item name from the FnbExpItem table
            $item = ReservationsExpItem::where('id', $this->item_id)->value('item');
...
$this->totalAmount = $this->lists->sum('amount');
...

    {
        $this->expenses = HotelExpense::where('section', $this->section)
            ->orderBy('id', 'desc')
            ->get();
        $items = ReservationsExpItem::orderBy("id", "desc")->get();
        $categories = ReservationsExpCategory::orderBy("id", "desc")->get();

        return view('livewire.reservations.reservations-make-expense', [
            'items' => $items,
