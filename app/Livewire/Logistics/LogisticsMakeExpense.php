...
use App\Models\LogisticsExpItem;
...
#[Title('Logistics | Make Expenses ')]
class LogisticsMakeExpense extends Component
{
    ...
    public $section  = 'Kitchen_Restaurant'; // I probably shouldnt hard code this
    public $category_id;
    public $item_id;
    public $amount;
    public $note;
    ...
    public function save()
    {
        try {
            ...
            // Get the item name from the LogisticsExpItem table
            $item = LogisticsExpItem::where('id', $this->item_id)->value('item');
...
$this->totalAmount = $this->lists->sum('amount');
...

    {
        $this->expenses = HotelExpense::where('section', $this->section)
            ->orderBy('id', 'desc')
            ->get();
        $items = LogisticsExpItem::orderBy("id", "desc")->get();
        $categories = LogisticsExpCategory::orderBy("id", "desc")->get();

        return view('livewire.logistics.logistics-make-expense', [
            'items' => $items,
