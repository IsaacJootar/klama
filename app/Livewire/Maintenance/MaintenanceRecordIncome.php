...
#[Title('Maintenance | Record Income ')]
class MaintenanceRecordIncome extends Component
{
    ...
    public function save()
    {
        try {
            ...
            // Get the item name from the MaintenanceIncItem table
            $item = MaintenanceIncItem::where('id', $this->item_id)->value('item');
   ...
   public function destroy($id)
{
...

    {
        $this->incomes = HotelIncome::where('section', $this->section)
            ->orderBy('id', 'desc')
            ->get();
        $items = MaintenanceIncItem::orderBy("id", "desc")->get();
        $categories = MaintenanceIncCategory::orderBy("id", "desc")->get();

        return view('livewire.maintenance.maintenance-record-income', [
            'items' => $items,
