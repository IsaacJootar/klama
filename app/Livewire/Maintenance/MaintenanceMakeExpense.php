...
#[Title('Maintenance | Make Expenses ')]
class MaintenanceMakeExpense extends Component
{
    ...
    public $section  = 'Maintenance'; // I probably shouldnt hard code this
    public $category_id;
    public $item_id;
    public $amount;
    public $note;
    ...
    public function save()
    {
        try {
            ...
            // Get the item name from the MaintenanceExpItem table
            $item = MaintenanceExpItem::where('id', $this->item_id)->value('item');
...
$this->totalAmount = $this->lists->sum('amount');
...

    {
        $this->expenses = HotelExpense::where('section', $this->section)
            ->orderBy('id', 'desc')
            ->get();
        $items = MaintenanceExpItem::orderBy("id", "desc")->get();
        $categories = MaintenanceExpCategory::orderBy("id", "desc")->get();

        return view('livewire.maintenance.maintenance-make-expense', [
            'items' => $items,
