...
#[Title('General Manager | General Record Income ')]
class GeneralRecordIncome extends Component
{
    ...
    public function save()
    {
        try {
            ...
            // Get the item name from the GeneralIncItem table
            $item = GeneralIncItem::where('id', $this->item_id)->value('item');
   ...
   public function destroy($id)
{
...

    {
        $this->incomes = HotelIncome::where('section', $this->section)
            ->orderBy('id', 'desc')
            ->get();
        $items = GeneralIncItem::orderBy("id", "desc")->get();
        $categories = GeneralIncCategory::orderBy("id", "desc")->get();

        return view('livewire.general.general-record-income', [
            'items' => $items,
